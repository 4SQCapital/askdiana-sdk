"""
OneDrive Connector — Ask DIANA Extension

A standalone Flask app that:
1. Receives install/uninstall webhooks from Ask DIANA
2. Handles Microsoft OAuth on behalf of users
3. Lists files from the user's OneDrive via Microsoft Graph API
...
# --- Service ---
onedrive_svc = OneDriveService(app.client)

# Register all connector routes (/api/auth/*, /api/files, /api/sync)
onedrive_svc.register_routes(app)
...
def on_uninstall():
    ...
    if install_id:
        try:
            onedrive_svc.disconnect(install_id)
