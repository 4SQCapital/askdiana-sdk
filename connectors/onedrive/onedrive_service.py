"""
OneDrive connector service using the Ask DIANA SDK.

Extends ``ConnectorService`` to provide OneDrive file listing
and syncing via OAuth2 (user authorizes their Microsoft account).
"""
...
class OneDriveService(ConnectorService):
    """Connector service for Microsoft OneDrive with OAuth.

    Users connect their Microsoft account via OAuth. Tokens are stored
    ...
    def __init__(self, client, client_id: Optional[str] = None, client_secret: Optional[str] = None):
        ...
        self.client_id = client_id or ONEDRIVE_CLIENT_ID
        self.client_secret = client_secret or ONEDRIVE_CLIENT_SECRET
        if not self.client_id or not self.client_secret:
            logger.warning(
                "ONEDRIVE_CLIENT_ID / ONEDRIVE_CLIENT_SECRET not set — OAuth will fail"
            )
    ...
    def get_auth_url(self, install_id: str, redirect_uri: str) -> str:
        """Build Microsoft OAuth URL with install_id as state."""
        return onedrive.build_auth_url(
    ...
    def handle_auth_callback(
        ...
        token_data = onedrive.exchange_code(
            code=code,
            client_id=self.client_id,
        ...
        user_info = onedrive.get_user_info(access_token)
    ...
    def disconnect(self, install_id: str) -> Dict[str, Any]:
        """Clear stored credentials (Microsoft doesn't support programmatic token revocation)."""
    ...
    def _get_valid_access_token(self, install_id: str) -> str:
        ...
        tokens = self.get_tokens(install_id)
        if not tokens:
            raise ValueError("Not connected to OneDrive. Please authorize first.")
        ...
        if tokens.get("expires_at", 0) < time.time() + 60:
            ...
            new_token_data = onedrive.refresh_access_token(
    ...
    def list_files(
        ...
        **kwargs: Any,
        ...
        files_data = onedrive.list_files(
            access_token=access_token,
            folder_id=folder_id,
    ...
    def download_file(
        ...
        **kwargs: Any,
        ...
        access_token = self._get_valid_access_token(install_id)
        return onedrive.download_file(access_token, file_id)
