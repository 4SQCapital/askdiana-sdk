...
# --- Service ---
dropbox_svc = DropboxService(app.client)

# Register all connector routes (/api/auth/*, /api/files, /api/sync)
dropbox_svc.register_routes(app)
...
def on_uninstall():
    ...
    if install_id:
        try:
            dropbox_svc.disconnect(install_id)
