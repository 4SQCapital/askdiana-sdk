"""
Dropbox connector service using the Ask DIANA SDK.

Extends ``ConnectorService`` to provide Dropbox file listing
and syncing via OAuth2 (user authorizes their Dropbox account).
"""
...
class DropboxService(ConnectorService):
    """Connector service for Dropbox with OAuth.

    Users connect their Dropbox account via OAuth. Tokens are stored
    ...
    def __init__(self, client, client_id: Optional[str] = None, client_secret: Optional[str] = None):
        ...
        if not self.client_id or not self.client_secret:
            logger.warning(
                "DROPBOX_CLIENT_ID / DROPBOX_CLIENT_SECRET not set — OAuth will fail"
            )
    ...
    def get_auth_url(self, install_id: str, redirect_uri: str) -> str:
        """Build Dropbox OAuth URL with install_id as state."""
        return dropbox_api.build_auth_url(
    ...
    def handle_auth_callback(
        ...
        token_data = dropbox_api.exchange_code(
            code=code,
            client_id=self.client_id,
        ...
        user_info = dropbox_api.get_user_info(access_token)
    ...
    def disconnect(self, install_id: str) -> Dict[str, Any]:
        """Revoke Dropbox tokens and clear stored credentials."""
        tokens = self.get_tokens(install_id)
        if tokens and tokens.get("access_token"):
            dropbox_api.revoke_token(tokens["access_token"])
    ...
    def _get_valid_access_token(self, install_id: str) -> str:
        ...
        tokens = self.get_tokens(install_id)
        if not tokens:
            raise ValueError("Not connected to Dropbox. Please authorize first.")
        ...
        if tokens.get("expires_at", 0) < time.time() + 60:
            ...
            new_token_data = dropbox_api.refresh_access_token(
                refresh_token=refresh_token,
                client_id=self.client_id,
                client_secret=self.client_secret,
            )
            tokens["access_token"] = new_token_data["access_token"]
            tokens["expires_at"] = time.time() + new_token_data.get("expires_in", 14400)
    ...
    def list_files(
        ...
        **kwargs: Any,
        ...
        if not folder_id:
            folder_id = self.get_config_value(install_id, "root_folder_path") or None

        files_data = dropbox_api.list_files(
            access_token=access_token,
    ...
    def download_file(
        ...
        **kwargs: Any,
        ...
        """Download a file from Dropbox (file_id is the path_lower)."""
        install_id = kwargs.get("install_id")
        if not install_id:
            raise ValueError("install_id required for download")
        access_token = self._get_valid_access_token(install_id)
        return dropbox_api.download_file(access_token, file_id)